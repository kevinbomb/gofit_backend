                                ...
                                <tbody>
                                    <tr v-for="(presensi, id) in presensig" :key="id">
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button v-if="presensi.WAKTU_PRESENSIG==null" @click.prevent="confirmKonfirmasi(presensi.NO_PRESENSIG)" class="btn btn-sm btn-outline-info ml-1">KONFIRMASI</button>
                                            <button v-else @click.prevent="confirmKonfirmasi(presensi.NO_PRESENSIG)" class="btn btn-sm btn-outline-danger disabled ml-1">KONFIRMASI</button>
...
export default {
    setup() {
        ...
        function getData(){
            ...
            axios.get('http://localhost:8000/api/presensiGym', {
                headers: {
        ...
        function konfirmasi(id) {
            ...
            axios.put(`http://localhost:8000/api/konfPresensiGym/${id}`,{
                headers: {
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem("token")}`
                }
                ...
                .then(() => {
                getData()
                alert("Presensi Berhasil, Transaksi Ditambah");
             }).catch(error => {
                 console.log(error.response.data)
             })
        ...
        function confirmKonfirmasi(id) {
            if (confirm("Are you sure you want to accept this gym appointment?")) {
                this.konfirmasi(id);
            }
        }
